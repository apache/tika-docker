...
case "$subcommand" in
  ...
  test-uat)
    # Run the tika-server REST UAT (release-tools/uat/run-uat.sh from tika-main)
    # against both images. Locate the script via $TIKA_MAIN or sibling checkout.
    ...
    if [[ ! -x "$uat_script" ]]; then
      die "UAT script not found or not executable: $uat_script (set TIKA_MAIN env var to your tika-main checkout)"
    fi
